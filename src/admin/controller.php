...
class OscampusController extends OscampusControllerBase
{
    ...
    /**
     * Temporary task for attempting to identify bad OSCampus URLs (work in progress)
     */
    public function gurulinks()
    {
        $db = OscampusFactory::getDbo();
        ...
        foreach ($tables as $table) {
            $idField = $db->setQuery("show columns from {$table} where `Key` = 'PRI' AND `Extra` = 'auto_increment'")
                ->loadColumn();
            if ($error = $db->getErrorMsg()) {
                OscampusFactory::getApplication()->enqueueMessage($error, 'error');
                return;
            }
            ...
            if (count($idField) == 1) {
                ...
                $where = array();
                foreach ($fields as $field) {
                    $where[] = $db->quoteName($field) . ' RLIKE ' . $db->quote('/classes/');
                }
                $query = $db->getQuery(true)
                    ->select($fields)
                    ->from($table)
                ...
                if ($rows = $db->setQuery($query)->loadAssocList()) {
                    $html[] = '<li>' . $table . ' - ' . count($rows) . ' Rows';
                    foreach ($rows as $row) {
                        $found = array();
                        foreach ($row as $fieldName => $value) {
                            if ($fieldName != $idField && $value) {
                                if (preg_match_all('#/classes/\S*?/\S*?/\S*?/\S*?/#ms', $value, $matches)) {
                                    $found = array_merge($found, $matches[0]);
                                }
                            }
                        }
                        if ($found) {
                            $html[] = '<ul>ID: ' . $row[$idField] . '<ul>';
                            $html[] = '<li>' . join('</li><li>', $found) . '</li>';
                            $html[] = '</ul></ul>';
                        }
                    }
                    $html[] = '</li>';
        ...
        $html[] = '<p>Total Runtime: ' . number_format((microtime(true) - $start), 1) . '</p>';
